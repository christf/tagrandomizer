...
/*
 * $Id: XMLOutputFactoryImpl.java,v 1.4 2006/05/18 05:54:27 sunithareddy Exp $
 * @(#)XMLOutputFactoryImpl.java    1.6 08/05/23
 *
...
public class XMLOutputFactoryImpl extends XMLOutputFactory {
	...
	private PropertyManager fPropertyManager = new PropertyManager(
	...
	public javax.xml.stream.XMLEventWriter createXMLEventWriter(
	...
	public javax.xml.stream.XMLEventWriter createXMLEventWriter(
			java.io.Writer writer) throws javax.xml.stream.XMLStreamException {
		return new XMLEventWriterImpl(createXMLStreamWriter(writer));
	}

	public javax.xml.stream.XMLStreamWriter createXMLStreamWriter(
			javax.xml.transform.Result result)
			throws javax.xml.stream.XMLStreamException {
		...
		if (result instanceof StreamResult) {
			...
			if (result.getSystemId() != null) {
				// this is not correct impl of SAXResult. Keep it for now for
				// compatibility
				return createXMLStreamWriter(new StreamResult(
						result.getSystemId()));
			} else {
				throw new java.lang.UnsupportedOperationException(
						"Result of type "
	...
	public javax.xml.stream.XMLStreamWriter createXMLStreamWriter(
			java.io.OutputStream outputStream, String encoding)
			throws javax.xml.stream.XMLStreamException {
		return createXMLStreamWriter(toStreamResult(outputStream, null, null),
				encoding);
	}
	...
	public void setProperty(String name, Object value)
		...
		if (name == null || value == null
				|| !fPropertyManager.containsProperty(name)) {
			throw new IllegalArgumentException("Property " + name
					+ "is not supported");
		}
		if (name == Constants.REUSE_INSTANCE
				|| name.equals(Constants.REUSE_INSTANCE)) {
			fReuseInstance = ((Boolean) value).booleanValue();
			if (DEBUG)
				System.out
						.println("fReuseInstance is set to " + fReuseInstance);
			...
			if (fReuseInstance) {
				...
				// REVISIT: Even in this case instance can be reused, by passing
				// PropertyManager
	...
	javax.xml.stream.XMLStreamWriter createXMLStreamWriter(
			javax.xml.transform.stream.StreamResult sr, String encoding)
			throws javax.xml.stream.XMLStreamException {
		// if factory is configured to reuse the instance & this instance can be
		// reused
		...
		try {
			if (fReuseInstance && fStreamWriter != null
				...
				if (DEBUG)
					System.out.println("reusing instance, object id : "
							+ fStreamWriter);
