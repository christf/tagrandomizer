 ...
 * implementation-specific properties. For example, you can get the underlying
 * <code>OutputStream</code> by casting an instance of this class to
 * <code>Map</code> and calling <code>getProperty(OUTPUTSTREAM_PROPERTY)</code>.
 * 
 * @author Neeraj Bajaj
 * @author K.Venugopal
...
public final class XMLStreamWriterImpl extends AbstractMap
	...
	/**
	 * This flag can be used to turn escaping off for content. It does not apply
	...
	/**
	 * Underlying OutputStream to which <code>fWriter</code> writes to. May be
	 * null if unknown.
	 */
	...
	/**
	 * Namespace context encapsulating user specified context and context built
	 * by the writer
	 */
	...
	private ElementStack fElementStack = new ElementStack(); // Change this
																// .-Venu
	 ...
	 * 
	 * @param outputStream
	 *            Underlying stream to write the bytes to
	 * @param props
	 *            Properties used by this writer
	 */
	 ...
	 * 
	 * @param outputStream
	 *            Underlying stream to write the bytes
	 * @param encoding
	 *            Encoding used to convert chars into bytes
	 * @param props
	 *            Properties used by this writer
	 */
	 ...
	 * 
	 * @param writer
	 *            Underlying writer to which chars are written
	 * @param props
	 *            Properties used by this writer
	 */
	...
	/**
	 * Creates a new instance of XMLStreamWriterImpl using a StreamResult. A
	 * StreamResult encasupates an OutputStream, a Writer or a SystemId.
	 * 
	 * @param writer
	 *            Underlying writer to which chars are written
	 * @param props
	...
	private void init() {
		...
		Boolean ob = (Boolean) fPropertyManager
				.getProperty(XMLOutputFactory.IS_REPAIRING_NAMESPACES);
		fIsRepairingNamespace = ob.booleanValue();
		ob = (Boolean) fPropertyManager
				.getProperty(Constants.ESCAPE_CHARACTERS);
		setEscapeCharacters(ob.booleanValue());
	}

	/**
	 * Reset this instance so that it can be re-used. Do not read properties
	 * again. The method <code>setOutput(StreamResult, encoding)</code> must be
	 ...
	 *            Indicates if properties should be read again
	 */
	 ...
	 * Check for OutputStream, Writer and then systemId, in that order.
	 * 
	 * @param sr
	 *            StreamResult encapsulating output information
	 * @param encoding
	 *            Encoding to be used except when a Writer is available
	 */
	...
	/**
	 * Utility method to create a writer when passed an OutputStream. Make sure
	 * to wrap an <code>OutputStreamWriter</code> using an
	 ...
	 * 
	 * @return boolean boolean value to indicate if this instance can be reused
	 *         or not
	 */
	...
	/**
	 * Return a prefix associated with specified uri, or null if the uri is
	 * unknown.
	 * 
	 * @param uri
	 *            The namespace uri
	 * @throws XMLStreamException
	 *             if uri specified is "" or null
	 */
	...
	/**
	 * Set the specified URI as default namespace in the current namespace
	 * context.
	 * 
	 * @param uri
	 *            Namespace URI
	...
	/**
	 * Sets the current <code>NamespaceContext</code> for prefix and uri
	 * bindings. This context becomes the root namespace context for writing and
	 * will replace the current root namespace context. Subsequent calls to
	 * setPrefix and setDefaultNamespace will bind namespaces using the context
	 * passed to the method as the root context for resolving namespaces. This
	 * method may only be called once at the start of the document. It does not
	 * cause the namespaces to be declared. If a namespace URI to prefix mapping
	 * is found in the namespace context it is treated as declared and the
	 * prefix may be used by the <code>XMLStreamWriter</code>.
	 * 
	...
	public void writeAttribute(String localName, String value)
		...
		try {
			...
			if (fIsRepairingNamespace) {
				Attribute attr = new Attribute(value); // Revisit:Dont create
														// new one's.
														// Reuse.-Venu
			...
			if (!fIsRepairingNamespace) {
				...
				if (!prefix.equals(XMLConstants.XML_NS_PREFIX)
					...
					if (fInternalNamespaceContext
						...
						String tmpURI = fInternalNamespaceContext
								.getURI(prefix);

						if (tmpURI != null && tmpURI != namespaceURI) {
							throw new XMLStreamException("Prefix " + prefix
									+ " is " + "already bound to " + tmpURI
									+ ". Trying to rebind it to "
									+ namespaceURI + " is an error.");
						}
					}
					fInternalNamespaceContext.declarePrefix(prefix,
							namespaceURI);
				}
				writeAttributeWithPrefix(prefix, localName, value);
			} else {
	 ...
	 * 
	 * If namespaceURI == null, then it is assumed to be equivilent to {@link
		...
		try {
			...
			if (fIsRepairingNamespace) {
				...
				qname.setValues(XMLConstants.DEFAULT_NS_PREFIX,
						XMLConstants.XMLNS_ATTRIBUTE, null,
			...
			namespaceURINormalized = fSymbolTable
					.addSymbol(namespaceURINormalized);
				...
				if (tmp != null && tmp != namespaceURINormalized) {
					throw new XMLStreamException(
							"xmlns has been already bound to " + tmp
									+ ". Rebinding it to "
									+ namespaceURINormalized + " is an error");
				}
			}
	...
	public void writeEmptyElement(String prefix, String localName,
		...
		try {
			...
			if (!fIsRepairingNamespace) {
				if (prefix == null) {
					throw new XMLStreamException("NamespaceURI " + namespaceURI
	 ...
	 * 
	 * If namespaceURI == null, then it is assumed to be equivilent to
	 * {@link XMLConstants.NULL_NS_URI}, i.e. there is no Namespace.
	 * 
	 * @param prefix
	 *            Prefix to bind.
	 * @param namespaceURI
	 *            NamespaceURI to declare.
	 * 
	 * @throws XMLStreamException
	 * 
	 * @see <a href="http://www.w3.org/TR/REC-xml-names/#defaulting"> Namespaces
	...
	public void writeNamespace(String prefix, String namespaceURI)
		...
		try {
			...
			if (prefix.equals(XMLConstants.XML_NS_PREFIX)
					&& namespaceURINormalized.equals(XMLConstants.XML_NS_URI))
				return;
	...
	public void writeStartDocument(String encoding, String version)
		...
		try {
			...
			if (fWriter instanceof OutputStreamWriter) {
				...
				streamEncoding = ((UTF8OutputStreamWriter) fWriter)
						.getEncoding();
			} else if (fWriter instanceof XMLWriter) {
				streamEncoding = ((OutputStreamWriter) ((XMLWriter) fWriter)
						.getWriter()).getEncoding();
			}

			if (streamEncoding != null
					&& !streamEncoding.equalsIgnoreCase(encoding)) {
				// If the equality check failed, check for charset encoding
				// aliases
				boolean foundAlias = false;
				Set aliases = Charset.forName(encoding).aliases();
				for (Iterator it = aliases.iterator(); !foundAlias
						&& it.hasNext();) {
	...
	private void writeXMLContent(String content) throws IOException {
		if ((content != null) && (content.length() > 0)) {
			writeXMLContent(content, fEscapeCharacters, // boolean = escapeChars
	 ...
	 * escaping character feature is turned off.
	 */
	private void writeXMLContent(String content, boolean escapeChars,
			boolean escapeDoubleQuotes) throws IOException {
				...
				for (int j = 0; j < fAttributeCache.size(); j++) {
					...
					if ((attr.prefix != null) && (attr.uri != null)) {
						if (!attr.prefix.equals("") && !attr.uri.equals("")) {
							String tmp = fInternalNamespaceContext
									.getPrefix(attr.uri);
	...
	private void correctPrefix(QName attr) {
		...
		if (prefix == null || prefix.equals("")) {
			...
			if (tmpPrefix == null) {
				...
				qname.setValues(prefix, XMLConstants.XMLNS_ATTRIBUTE, null, uri);
				fNamespaceDecls.add(qname);
				fInternalNamespaceContext.declarePrefix(
						fSymbolTable.addSymbol(prefix), uri);
			}
		}
	...
	private boolean isDefaultNamespace(String uri) {
		String defaultNamespace = fInternalNamespaceContext
				.getURI(DEFAULT_PREFIX);
	...
	private boolean checkUserNamespaceContext(String prefix, String uri) {
		if (fNamespaceContext.userContext != null) {
			String tmpURI = fNamespaceContext.userContext
	...
	/**
	 * Correct's namespaces as per requirements of isReparisingNamespace
	 * property.
	 */
	protected void repair() {
		...
		if (!isDeclared(currentElement)) {
			if ((currentElement.prefix != null) && (currentElement.uri != null)) {
				if ((!currentElement.prefix.equals(""))
		...
		for (i = 0; i < fAttributeCache.size(); i++) {
			attr = (Attribute) fAttributeCache.get(i);
			/*
			 * If 'attr' is an attribute and it is in no namespace(which means
			 * that prefix="", uri=""), attr's namespace should not be redinded.
		...
		for (i = 0; i < fNamespaceDecls.size(); i++) {
			...
			if (qname != null) {
				fInternalNamespaceContext
						.declarePrefix(qname.prefix, qname.uri);
			}
		}
	 ...
	 * prefixthat is bound to the namespace URIs of those attributes.
	 */
		...
		for (int i = 0; i < fNamespaceDecls.size(); i++) {
			...
			if (decl1 != null) {
				for (int j = i + 1; j < fNamespaceDecls.size(); j++) {
					...
					// QName.equals relies on identity equality, so we can't use
					// it,
					// because prefixes aren't interned
					if (decl2 != null && decl1.prefix.equals(decl2.prefix)
	...
	/*
	 * If an element or attribute name is bound to a prefix and there is a
	 * namespacedeclaration that binds that prefix to a different URI then that
	 * namespace declarationis either removed if the correct mapping is
	 * inherited from the parent context of that element,or changed to the
	...
	void repairNamespaceDecl(QName attr) {
		...
		for (int j = 0; j < fNamespaceDecls.size(); j++) {
			...
			if (decl != null) {
				if ((attr.prefix != null)
						&& (attr.prefix.equals(decl.prefix) && !(attr.uri
								.equals(decl.uri)))) {
	...
	protected class ElementStack {
		...
		public ElementState push(String prefix, String localpart,
			...
			fElements[fDepth].setValues(prefix, localpart, rawname, uri,
					isEmpty);
		 ...
		 * we dont need to set the value for every end element we encouter. For
