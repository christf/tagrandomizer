 ...
 * is designed to run on JDK version 1.1 and later including JVMs that perform
 * early linking like the Microsoft JVM in IE 5. Note however that it must be
 * compiled on a JDK version 1.2 or later system since it calls
 * Thread#getContextClassLoader(). The code also runs both as part of an
...
final class FactoryFinder {
	 ...
	 *            Package private so this code can be shared.
	 */
	static Object find(String factoryId, ClassLoader classLoader,
		...
		try {
			...
			// if first option fails due to any reason we should try next option
			// in the
			// look up algorithm.
		}
		...
		try {
				...
				synchronized (cacheProps) {
					if (firstTime) {
						...
						if (SecuritySupport.doesFileExist(f)) {
							...
							cacheProps.load(SecuritySupport
									.getFileInputStream(f));
						}
					}
				}
			}
			...
			if (factoryClassName != null) {
				if (debug)
					dPrint("found in $java.home/stax.properties, value="
							+ factoryClassName);
